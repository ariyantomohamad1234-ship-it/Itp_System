...
@section('styles')
<style>
    .manage-project-container { width:100%; max-width:100%; box-sizing:border-box; padding:0; margin:0; }
    .section-card { background:var(--card); border:1px solid var(--border); border-radius:1rem; margin-bottom:1rem; overflow:visible; width:100%; box-sizing:border-box; }
    .section-header { padding:1rem 1.25rem; border-bottom:1px solid var(--border); display:flex; justify-content:space-between; align-items:center; cursor:pointer; transition:background 0.2s; gap:8px; min-width:0; flex-wrap:wrap; }
    .section-header:hover { background:#f8fafc; }
    .section-header .chevron-icon { transition:transform 0.3s ease; font-size:0.7rem; color:var(--text-muted); flex-shrink:0; }
    .section-header[aria-expanded="true"] .chevron-icon { transform:rotate(180deg); }
    .section-body { padding:1.25rem; overflow-x:auto; -webkit-overflow-scrolling:touch; }
    .add-form { display:flex; gap:8px; align-items:end; flex-wrap:wrap; }
    .add-form .form-control, .add-form .form-select { font-size:0.85rem; padding:0.45rem 0.75rem; border-radius:0.5rem; min-width:0; box-sizing:border-box; }
    .itp-form-card { background:#f8fafc; border:1px dashed #cbd5e1; border-radius:0.75rem; padding:1rem; margin-bottom:0.75rem; }
    .itp-form-card .form-label { font-size:0.7rem; font-weight:700; color:var(--text-muted); text-transform:uppercase; letter-spacing:0.5px; margin-bottom:0.25rem; }
    .itp-form-row { display:grid; grid-template-columns:1fr 1fr; gap:0.75rem; margin-bottom:0.75rem; }
    .itp-code-row { display:grid; grid-template-columns:100px 1fr; gap:0.75rem; margin-bottom:0.75rem; }
    .itp-val-row { display:grid; grid-template-columns:repeat(4,1fr) auto; gap:0.5rem; align-items:end; }
    .itp-val-row .val-col { text-align:center; }
    .itp-val-row .val-col .form-select { font-size:0.8rem; padding:0.35rem 0.4rem; text-align:center; }
    .val-label { font-size:0.6rem; font-weight:700; text-transform:uppercase; letter-spacing:1px; color:var(--text-muted); margin-bottom:0.2rem; }
    .tree-node { margin-left:1.5rem; padding-left:1rem; border-left:2px solid #e2e8f0; }
    .tree-item { display:flex; justify-content:space-between; align-items:center; padding:0.6rem 0.85rem; border:1px solid #f1f5f9; border-radius:0.625rem; margin-bottom:0.35rem; font-size:0.85rem; transition:all 0.2s; cursor:pointer; background:#fff; gap:8px; min-width:0; }
    .tree-item:hover { background:#f8fafc; border-color:#e2e8f0; box-shadow:0 2px 8px -2px rgba(0,0,0,0.04); }
    .itp-item { display:flex; justify-content:space-between; align-items:center; padding:0.5rem 0.75rem; border:1px solid #f1f5f9; border-radius:0.5rem; margin-bottom:0.3rem; font-size:0.82rem; background:#fff; transition:background 0.2s; gap:8px; }
    ...
    .val-w { background:#fef3c7; color:#92400e; } .val-rv { background:#dbeafe; color:#1e40af; }
    .val-dash { background:#f1f5f9; color:#94a3b8; } .val-na { background:#fee2e2; color:#991b1b; }
    .assigned-user { display:inline-flex; align-items:center; gap:0.5rem; background:#fff; border:1px solid #cbd5e1; padding:5px 10px 5px 12px; border-radius:2rem; font-size:0.8rem; margin:3px; transition:all 0.2s; }
    .assigned-user:hover { background:#f1f5f9; border-color:#cbd5e1; }
    .assigned-user .btn-unassign { background:none; border:none; color:#ef4444; cursor:pointer; font-size:0.7rem; padding:0 2px; }
    .assigned-user .btn-unassign:hover { color:#dc2626; }
    .schedule-item { background:linear-gradient(135deg,#f0f4ff,#f5f3ff); border:1px solid #c7d2fe; border-radius:0.75rem; padding:1rem; margin-bottom:0.5rem; }
    .schedule-item label { font-size:0.72rem; font-weight:600; color:#4338ca; margin-bottom:0.2rem; }
    .project-header { background:linear-gradient(135deg,#0f172a,#1e293b); border-radius:1rem; padding:1.5rem 1.75rem; color:#fff; margin-bottom:1.25rem; position:relative; overflow:hidden; }
    .project-header::before { content:''; position:absolute; top:-50%; right:-15%; width:250px; height:250px; background:radial-gradient(circle,rgba(99,102,241,0.12) 0%,transparent 60%); border-radius:50%; }
    ...
    @media (max-width:768px) {
        .section-header { flex-direction:row; align-items:center; padding:0.875rem 1rem; }
        .itp-form-row { grid-template-columns:1fr; } .itp-code-row { grid-template-columns:1fr; }
        .itp-val-row { grid-template-columns:repeat(2,1fr); } .itp-val-row .btn-submit-col { grid-column:1/-1; }
        .tree-node { margin-left:0.75rem; padding-left:0.75rem; }
        .add-form { flex-direction:column; } .add-form .form-control,.add-form .form-select,.add-form button { width:100%; }
        .section-body { padding:1rem; }
    }
    @media (max-width:576px) {
        .project-header { padding:1.25rem 1rem; border-radius:0.75rem; margin-bottom:1rem; }
        .project-header h5 { font-size:1.1rem; } .project-header .meta { font-size:0.75rem; }
        .section-header { padding:0.75rem 0.875rem; } .section-body { padding:0.875rem; }
        .tree-node { margin-left:0.5rem; padding-left:0.5rem; }
    }
</style>
@endsection
            ...
            <div>
                ...
                <div class="meta">
                    ...
                    @if($project->tanggal_kontrak)
                        &nbsp;&bull;&nbsp; <i class="fas fa-file-signature me-1"></i>Kontrak: {{ $project->tanggal_kontrak->format('d M Y') }}
                    @endif
                    @if($project->tanggal_mulai)
                        &nbsp;&bull;&nbsp; <i class="fas fa-calendar-alt me-1"></i>Mulai: {{ $project->tanggal_mulai->format('d M Y') }}
                    @endif
                    @if($project->deadline)
                        &nbsp;&bull;&nbsp; <i class="fas fa-flag-checkered me-1"></i>Deadline: {{ $project->deadline->format('d M Y') }}
                    @endif
                </div>
            </div>
                            ...
                            <div class="mb-2">
                                ...
                                <div class="collapse" id="sub-{{ $sub->id }}">
                                    <div class="tree-node mt-1">
                                        <div class="itp-form-card">
                                            <form action="/admin/itps" method="POST">
                                                ...
                                                <div class="itp-val-row">
                                                    @foreach(['yard'=>'Yard','class'=>'Class','os'=>'OS','stat'=>'Stat'] as $key=>$label)
                                                    <div class="val-col">
                                                        <div class="val-label">{{ $label }}</div>
                                                        <select name="{{ $key }}_val" class="form-select form-select-sm" required>
                                        ...
                                        @forelse($sub->itps as $itp)
                                        @php $valClass = fn($v) => match(strtoupper($v)) { 'W' => 'val-w', 'RV' => 'val-rv', 'NA' => 'val-na', default => 'val-dash' }; @endphp
                                        <div class="itp-item">
                                            <div class="itp-info">
                                                <span class="asm-code">{{ $itp->assembly_code }}</span>
                                                @if($itp->assembly_description)<small class="text-muted ms-1" title="{{ $itp->assembly_description }}"><i class="fas fa-info-circle"></i></small>@endif
                                                <span class="text-muted mx-1">›</span><span class="itp-code">{{ $itp->code }}</span>
                                                <span class="text-muted mx-1">—</span><span class="itp-desc">{{ $itp->item }}</span>
                                            </div>
                                            <div class="itp-vals">
                                                <span class="badge-val {{ $valClass($itp->yard_val) }}">Y:{{ $itp->yard_val }}</span>
                                                <span class="badge-val {{ $valClass($itp->class_val) }}">C:{{ $itp->class_val }}</span>
                                                <span class="badge-val {{ $valClass($itp->os_val) }}">O:{{ $itp->os_val }}</span>
                                                <span class="badge-val {{ $valClass($itp->stat_val) }}">S:{{ $itp->stat_val }}</span>
                                                <form action="/admin/itps/{{ $itp->id }}" method="POST" class="d-inline" onsubmit="return confirm('Hapus?')">
                                                    @csrf @method('DELETE')
                                                    <button class="btn btn-outline-danger btn-sm" style="font-size:0.55rem;padding:1px 6px"><i class="fas fa-times"></i></button>
                                                </form>
                                            </div>
                                        ...
                                        @empty
                                            <div class="text-muted text-center py-2" style="font-size:0.8rem"><i class="fas fa-info-circle me-1"></i>Belum ada ITP</div>
                                        @endforelse
                                    </div>
                                </div>
                            </div>
